# 단순회귀모형의 추론 {#simple-reg-infer}

회귀모형의 추론에서 우리가 살펴볼 내용은 회귀계수에 대한 신뢰구간 추정 및 검정, 그리고 
설명변수의 값이 주어졌을 때 반응변수의 조건부 평균에 대한 구간추정 등이다. 
또한 분산분석 개념을 이용한 회귀모형의 유의성 검정에 대해서도 살펴볼 것이다 
...
주어진 자료를 근거로 만일 $H_{0}$ 을 기각할 수 없다면, 식 \@ref(eq:simple-ch3-1)에서 설정된 모형이 $Y_{i}=\beta_{0}+\varepsilon_{i}$ 로 변경되는 것이며, 따라서 설명변수 $X$가 반응변수 $Y$의 변동 설명하는데 아무런 기여를 할 수 없다는 것을 의미하게 된다.
...
따라서 $\hat{\beta}_{0}$ 의 표본분포는 다음과 같이 나타낼 수 있다.  

\begin{equation}
\hat{\beta}_{0} ~ N\left(\beta_{0},~ \sigma^{2}\left(\frac{1}{n}+\frac{\overline{X}^{2}}{\sum_{i=1}^{n}(X_{i}-\overline{X})^{2}}\right) \right)
\end{equation}
