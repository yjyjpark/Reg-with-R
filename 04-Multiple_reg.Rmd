...
**2.** 데이터 프레임 `states`는 다음과 같이 8개 변수로 구성되어 있다. 
...
변수 `Murder`을 반응변수로 하고, 나머지 변수를 설명변수로 하는 회귀모형을 적합한 결과는 다음과 같다. 
...
- 모형 `fit_s`에 대한 회귀모형 유의성 검정을 실시해 보자. 주어진 결과물에서 검정통계량의 값과 p-value를 구하고, 검정 결과의 의미를 해석하라.  


- 모형 `fit_s`의 개별 회귀계수 검정 결과 중 5% 유의수준에서 유의적인 변수의 검정통계량 값과 p-value를 주어진 결과물에서 각각 구하라.


- 모형 `fit_s`의 결정계수와 수정결정계수를 주어진 결과물에서 각각 구하고, 그 의미를 설명하라. 


**3.** 데이터 파일 `car_df.csv`에는 234대 차량의 연비(`mpg`)와 배기량(`displ`), 변속기 종류(`trans`)가 입력되어 있다. 웹 서버 `https://raw.githubusercontent.com/yjyjpark/Reg-with-R/main/Data/`에 있는 자료를 불러와서 데이터 프레임 `car_df`에 할당하고, 처음 3줄을 출력한 결과는 다음과 같다.

```{r, include=FALSE}
car_df <- ggplot2::mpg |> 
  mutate(trans = if_else(substr(trans, 1, 1) == "a", "auto", "manual")) |> 
  select(mpg = cty, trans, displ)
```

```{r, echo=FALSE}
car_df <- read_csv("https://raw.githubusercontent.com/yjyjpark/Reg-with-R/main/Data/car_df.csv")
```
...
변수 `mpg`를 반응변수로 하고, 설명변수로는 연속형 변수인 `displ`과 범주형 변수인 `trans`를 사용하는 회귀모형을 적합하고자 한다. 변수 `trans`는 `auto` 또는 `manual`을 값으로 갖고 있다. 

- 첫 번째 회귀모형 `car_fit_1`의 적합 결과는 다음과 같다. 모형 `car_fit_1`의 모든 회귀계수 추정값의 의미를 해석하라. 
...
- 두 번째 회귀모형 `car_fit_2`의 적합 결과는 다음과 같다. 모형 `car_fit_2`의 모든 회귀계수 추정값의 의미를 해석하라.
...
**4.** 데이터 파일 `auto_df.csv`에는 302대 차량의 연비와 관련된 자료가 입력되어 있다. 웹 서버 `https://raw.githubusercontent.com/yjyjpark/Reg-with-R/main/Data/`에 있는 자료를 R로 불러와서 데이터 프레임 `auto_df`에 할당하고, 처음 3줄을 출력한 결과는 다음과 같다.  
...
- 변수 `mpg`를 반응변수로 하는 회귀모형을 적합하고자 한다. 설명변수는 나머지 6개 변수 중 Best subset selection으로 선택하고자 한다. 다음의 결과를 근거로 최적 변수를 선택하라. 
