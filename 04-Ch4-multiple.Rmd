...
반면에, 검정통계량의 분자인 $(TSS-RSS)/k$ 는 $H_{0}$ 이 사실인 경우에는 $\sigma^{2}$ 의 불편추정량이 되지만, $H_{0}$ 이 사실이 아닌 경우에는 $\sigma^{2}$ 을 과대 추정하게 된다.
즉, $H_{0}$이 사실이 아니면 다음과 같이 된다. 

\begin{equation}
E((TSS-RSS)/k) > \sigma^{2}
\end{equation}

따라서 검정통계량이 1보다 상당히 큰 값이 되면 $H_{0}$이 사실이 아닐 가능성이 높게 된다.

또한 $H_{0}$이 사실인 경우, 검정통계량의 분모와 분자는 각각 자유도가 $(n-k-1)$ 과 $k$ 이 $\chi^{2}$ 분포를 하게 되며, 서로 독립이 된다.
따라서 검정통계량은 $H_{0}$이 사실인 경우 자유도가 $(k, ~n-k-1)$ 인 $F$ 분포를 하기 때문에, $F$ 분포에서 기각역이 결정된다.
